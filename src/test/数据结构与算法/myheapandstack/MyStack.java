...
class MyArrayStack<Item> implements MyStack<Item>{
    ...
    **/
    private void judgeSize(){
        if (n >= a.length ){  // 当 n（实际元素个数）等于 a.length（数组容量）时，表示数组已经满载，需要先扩容
            resize(a.length * 2); // 扩容为原数组的2倍
        }else if(n >0  && n == a.length / 2){
            // 当 n（实际元素个数）等于 a.length（数组容量）的一半时，表示数组已经半满，需要先缩容，因为数组new出来的内存并不会自动释放，所以需要手动释放。
            resize(a.length / 2);
        }
    }
    /**
    * @Description: 将旧数组 a 中的元素复制到新数组 temp
     * @param size 扩容后的数组长度
    **/
    private void resize(int size){ // 扩容O(n)
        Item[] temp = (Item[]) new Object[size]; // 临时数组
        for (int i = 0; i < n; i++) {
            // 将旧数组 a 中的元素复制到新数组 temp 的对应位置。
            temp[i] = a[i];
        }
        a = temp; // 将新数组赋值给 a
    }
    ...
    public Item pop() { // O(1)
        ...
        Item item = a[--n]; // 把栈顶元素删除出栈，--n这个表达式的值为n-1，即递减后的值，也就是当前栈顶元素第二个元素，也是数组的倒数第二个元素。
        a[n] = null; // 释放栈顶元素引用：将删除的元素置为 null，以方便GC回收。n是数组长度，执行 --n 后，n 的值变为 n-1，这里是原始栈顶元素，即数组的倒数第一个元素。
        return item; // 返回
    }
